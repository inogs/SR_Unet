...
def parse_input_parameters():
    parser = argparse.ArgumentParser(
        description="Compute monthly mean/std statistics from per-month file lists."
    )
    parser.add_argument(
        "-c",
        "--config",
        default=os.path.join(os.path.dirname(__file__), "conf.files.dir", "conf_compute_monthly_stats.json"),
...
if __name__ == "__main__":
    ...
    conf = read_compute_monthly_stats_conf(args.config)
    ...
    for month, month_id in MONTH_IDS.items():
        ...
        stat_output_path = os.path.join(output_folder_path, f"stat.{month_id}.{month}.txt")

        if os.path.exists(stat_output_path):
            print(f"Output file {stat_output_path} already exists. Skipping computation for month {month}.")
            with open(stat_output_path) as f:
                mu = float(f.readline())
