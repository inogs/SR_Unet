# IDEA SIMONE:
# per ogni colonna (i,j) Cadeau:
# trovi le 4 celle Copernicus vicine
# trovi la loro profondità massima disponibile
# se new_dep[z] supera quella profondità:
# NON usare interpolazione verticale
# usa direttamente il valore dell’ultima cella valida della cella Copernicus più profonda
...
def interpolate_3d(values2interp, old_lon, old_lat, old_dep, new_grid, var_grid):
    ...
    masked_data = np.ma.masked_invalid(values2interp)
    ...
    for k in range(n_dep_old):
        ...
        if np.any(mask):
            points_valid = np.column_stack([LAT_old[mask], LON_old[mask]])
    ...
    # STEP 2: VERTICAL INTERPOLATION -> lineare + correzione 'pozzi'
    ...
    for i in range(n_lat_new):
        for j in range(n_lon_new):
            ...
            # COPERNICUS LOCAL DEPTH LIMIT (4 celle)
            lat0 = new_lat[i]
            lon0 = new_lon[j]

            ilat = np.argpartition(np.abs(old_lat - lat0), 2)[:2]
            ilon = np.argpartition(np.abs(old_lon - lon0), 2)[:2]

            cop_local_max = -np.inf

            for ii in ilat:
                for jj in ilon:

                    col = values2interp[:, ii, jj]
                    v = np.isfinite(col)

                    if np.any(v):
                        kk = np.where(v)[0][-1]
                        cop_local_max = max(cop_local_max, old_dep[kk])

            if not np.isfinite(cop_local_max):
                cop_local_max = old_dep[last_valid]

            # INTERPOLATION
            spline = make_interp_spline(old_dep, profile, k=1)
            y = spline(new_dep)

            y[new_dep < old_dep.min()] = profile[0]

            # cadeau depth vs copernicus depth
            too_deep = new_dep > cop_local_max

            if np.any(too_deep):
                y[too_deep] = profile[last_valid]
                n_cop_fallback += 1
    ...
    return interp_data, n_cop_fallback, n_unfixable
            ...
            with nc.Dataset(file_path, "r") as source_nc:
                ...
                # Create a new NetCDF file for writing
                    with nc.Dataset(tmp_file_path, "w") as dest_nc:
                        ...
                        interpolated_array, n_cop_fallback, n_unfixable = interpolate_3d(old_data, old_lon, old_lat, old_dep, grid_nc, var_grid)

                        if rank == 0:
                            with open(os.path.join(output_path, "copernicus_fallback_stats.txt"), "a") as f:
                                f.write(f"{source_file_name} | fallback: {n_cop_fallback} | unfixable: {n_unfixable}\n")
